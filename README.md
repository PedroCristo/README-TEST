...
To save the player name, city, score and date I am using a google sheet that is connected to the code through the Google Drive and Google Sheet API by the Google Cloud Platform. This method allows me to send and receive data as I had access to the Google Sheet API credentials and I added in the Config Vars this credentials when I was deploying the project in the Heroku. As a sensitive data we should add the creds.json in the Git ignore file to make sure that this credentials will be never push to the repository.
